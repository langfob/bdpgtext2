...
- The basic structure of all of the work in this project is that 
    ...
    - created 6 batches of 500 problems each and ran the reserve selectors on every problem, then
    - measured the solution cost error and representation shortfall error in each reserve selector's solution, then 
...
## Shiny supplement has a bug that wasn't there before

Not sure what this is about.  Here's the error message when I do the knit:

```
processing file: p9_v01_all_combined____Shiny_app__supplemental_information.Rmd
  |.............................       |  82% [plotBoxplot1pctTotOutErrFull]   
...
Backtrace:
 ...
 25.                   \-watcher$print_value(ev$value, ev$visible, envir)
 26.                     +-base::withVisible(handle_value(handler, value, visible, envir))
 27.                     \-evaluate:::handle_value(handler, value, visible, envir)
 28.                       \-handler$value(value, visible)
 29.                         \-knitr (local) fun(x, options = options)
 30.                           +-base::withVisible(knit_print(x, ...))
 31.                           +-knitr::knit_print(x, ...)
 32.                           \-knitr:::knit_print.default(x, ...)
 33.                             \-knitr::normal_print(x)
 34.                               +-base::print(x)
 35.                               \-ggplot2:::print.ggplot(x)
 36.                                 +-ggplot2::ggplot_build(x)
 37.                                 \-ggplot2:::ggplot_build.ggplot(x)
 38.                                   \-layout$setup(data, plot$data, plot$plot_env)
 39.                                     \-ggplot2 (local) setup(..., self = self)
 40.                                       \-self$facet$compute_layout(data, self$facet_params)
 41.                                         \-ggplot2 (local) compute_layout(..., self = self)
 42.                                           \-ggplot2::combine_vars(data, params$plot_env, vars, drop = params$drop)
 43.                                             \-cli::cli_abort("Faceting variables must have at least one value.")
 44.                                               \-rlang::abort(...)

Quitting from p9_v01_all_combined____Shiny_app__supplemental_information.Rmd:590-603 [plotBoxplot1pctTotOutErrFull]
Execution halted
```
...
There is a command in some of the plotting of predictions (in the file "v1a_paper_3_fitting_functions__returning_fitting_plots_instead_of_displaying_them.R") that says:
"ggplot (aes (value, reorder (row, value), color = value > 0))".
Maybe that has something to do with getting SA and SA_SS in the correct order since the help for the reorder function says:
...
See one strategy for splitting into independent groups in file:
"/R/v2_paper_3_cv_test_train_splitting_functions.R"
