...
#-----  path variables  -----# 
  ...
  prep_session_uuid: "4c261db9-913b-4a1b-abc1-88cd1cbd0665"    #  UUID printed by p9_v01_prep_data_for_p8_to_load_from_files.Rmd
  verbose_verify_prov_reads: FALSE    #  If TRUE, resolve_prov_file() prints a VERIFIED line for each of the 8 prepped inputs it loads
#-----  golden-master capture variables (fitting/eval refactor bootstrap; see planning/bdpg_fitting_refactor_plan.md) -----#
  capture_golden_master: FALSE    #  If TRUE, writes frozen input snapshots + all_fitting_scores_df under tests/fixtures/ for the new tidymodels pipeline's golden-master equivalence tests.  Inert (no behavior change) when FALSE.  See DECISIONS.md.
  force_golden_master: FALSE    #  If TRUE, allows capture_golden_master to overwrite an existing finalized golden record.  Otherwise the run aborts (before any fitting or writes) if one already exists, instead of silently regenerating it.
#-----  plotting variables  -----# 
...
#-----  new fitting/eval pipeline variables (parallel-build refactor; see planning/bdpg_fitting_refactor_plan.md) -----#
  use_new_fitting_pipeline: TRUE    #  If TRUE, wired subsections use the new tidymodels fitting/eval pipeline (R/v1_paper_9_fitting_and_eval_pipeline.R) instead of the old one.  Old path is the default and is byte-for-byte unchanged when FALSE.  See DECISIONS.md.
  save_final_model: FALSE    #  Global gate (plan §8): fit + save a versioned final model.  Stubbed with a TODO this round -- see DECISIONS.md (2026-06-28) and FUTURE_CHATS.md FC-4.  Quiet default; only reachable when use_new_fitting_pipeline is TRUE.
#-----  misc variables  -----#
...
library (dataprov)    #  For read-side provenance resolution of prepped inputs.
library (DBI)
```
...
source (file.path (proj_dir, "/R/p8.unifiedDataLoading.v01.R"), 
         local = knitr::knit_global())

#--------------------

source (file.path (proj_dir, "/R/p8.preprocessingForLearning.v01.R"), 
         local = knitr::knit_global())

source (file.path (proj_dir, "/R/p8.BoxCoxAndNormalizingFunctions.v01.R"), 
         local = knitr::knit_global())

source (file.path (proj_dir, "/R/v1_paper_3_plotting_and_evaluation_functions.R"), 
         local = knitr::knit_global())

source (file.path (proj_dir, "/R/v1_paper_3_utility_functions.R"), 
         local = knitr::knit_global())

source (file.path (proj_dir, "/R/v1_paper_3_fitting_functions.R"), 
         local = knitr::knit_global())

#  Parallel-build fitting/eval refactor (planning/bdpg_fitting_refactor_plan.md).
#  Used only when params$use_new_fitting_pipeline is TRUE (see the wired
#  subsection(s) below); harmless to source unconditionally otherwise.
source (file.path (proj_dir, "/R/v1_paper_9_fitting_and_eval_pipeline.R"),
         local = knitr::knit_global())

source (file.path (proj_dir, "/R/p8.matildaFunctions.v01.R"), 
         local = knitr::knit_global())
...
```{r initProvenanceTracking, include=FALSE}

#  Open the dataprov registry for resolving prepped inputs pinned to
#  params$prep_session_uuid (the session UUID printed by
#  p9_v01_prep_data_for_p8_to_load_from_files.Rmd).

source (file.path (proj_dir, "R/provenance_helpers.R"),
         local = knitr::knit_global())

reg = dataprov::prov_registry_open (file.path (proj_dir, "Data", "dataprov_registry"))
```

```{r goldenMasterOverwriteGuard, include=FALSE}

#  Overwrite guard for the golden-master capture (see
#  planning/bdpg_fitting_refactor_plan.md, "Golden-master specifics").  Evaluated
#  here, before any data loading or fitting, so a capture_golden_master = TRUE run
#  aborts immediately if a golden record already exists, instead of doing hours of
#  fitting first.  Inert when capture_golden_master is FALSE (the default).
#
#  Not implemented as an interactive prompt: interactive() is FALSE under
#  rmarkdown::render(), so readline() would not block and a prompt would silently
#  fall through.
#
#  Uses a DEDICATED registry under tests/fixtures/dataprov_registry, not the
#  production Data/dataprov_registry -- Data/ is a symlink out to the (separately
#  ignored) bdpgtext/Data directory and so is not part of this git repo, while the
#  golden-master fixtures must be committed here.

if (isTRUE (params$capture_golden_master))
    {
    fixture_root        = file.path (proj_dir, "tests", "fixtures")
    golden_dir          = file.path (fixture_root, "golden")
    golden_registry_dir = file.path (fixture_root, "dataprov_registry")

    golden_check_reg = open_or_init_registry (golden_registry_dir)

    existing_golden_records =
        dataprov::prov_list (golden_check_reg,
                             tags = list (tib = "all_fitting_scores_df",
                                         file_extension = "rds"))

    DBI::dbDisconnect (golden_check_reg$conn)

    golden_already_exists =
        (nrow (existing_golden_records) > 0) &&
        any (existing_golden_records$status == "finalized")

    if (golden_already_exists && ! isTRUE (params$force_golden_master))
        stop (paste0 ("params$capture_golden_master is TRUE, but a finalized ",
                      "'all_fitting_scores_df' golden record already exists in\n    ",
                      golden_registry_dir, "\n",
                      "Refusing to overwrite it.  Set params$force_golden_master = TRUE ",
...
resolved_path = resolve_prov_file (reg, params$prep_session_uuid,
                                   "filtered_full_initial_exp_tib", ext = "csv",
                                   verbose_verify = params$verbose_verify_prov_reads)
filtered_full_initial_exp_tib =
    load_file_into_tibble (file_name = basename (resolved_path),
                           base_path = dirname (resolved_path))

#--------------------

resolved_path = resolve_prov_file (reg, params$prep_session_uuid,
                                   "p2_app_wrap_tib", ext = "csv",
                                   verbose_verify = params$verbose_verify_prov_reads)
p2_app_wrap_tib =
    load_file_into_tibble (file_name = basename (resolved_path),
                           base_path = dirname (resolved_path))
...
resolved_path = resolve_prov_file (reg, params$prep_session_uuid,
                                   "p3_working_train_df__before_any_preprocessing", ext = "csv",
                                   verbose_verify = params$verbose_verify_prov_reads)
...
resolved_path = resolve_prov_file (reg, params$prep_session_uuid,
                                   "p3_working_test_df__before_any_preprocessing", ext = "csv",
                                   verbose_verify = params$verbose_verify_prov_reads)
...
```{r closeProvenanceTracking, include=FALSE}

#  All 8 provenance-resolved reads are done; close the registry connection.
...
filtered_full_initial_exp_tib <- filtered_full_initial_exp_tib %>%
  ...
  mutate(rs_method_name_fac = convert_rs_method_name_to_ordered_factor(rs_method_name))
...
We address these issues by developing a procedure for ‘wrapping’ one rank abundance distribution around another in a way that guarantees that the original base problem’s solution is still the solution to the *wrapped* problem but allows the final problem to have an arbitrary rank abundance distribution. Moreover, we can specify the proportion of the wrapped problem’s landscape taken up by the correct solution. This may mean that we are unable to generate problems as difficult as the hardest benchmark problems, but our results show that we can still generate problems with a wide range of difficulty for some reserve selectors.
...
Next, we examine how to create a problem where the correct solution occupies a specified proportion of the wrapped problem’s landscape, e.g., 40% to keep this example small. In this case, we need to have 10 PUs in the wrapped problem landscape since there were 4 PUs in the base problem’s solution. So, we need to add 4 more PUs to the base problem’s 6 and decide how to spread the 16 total occurrences of the 4 new species across the new, larger landscape.
...
Here, we used lognormal distributions as an example wrapping distribution. While there is no consensus on what is a theoretically correct rank abundance distribution, we used lognormal distributions as our example distribution because they have at least been discussed by ecologists in relation to theoretical models for rank abundance distributions (@stevens2009). Note that while the lognormal distribution is undoubtedly a more realistic rank abundance distribution over the *full* set of species in a particular location than model RB’s uniform distribution, the set of species considered in a reserve selection problem is never the full set of species occurring at that location.  Moreover, the practitioner can arbitrarily choose *any* subset of the species to include and therefore, the abundance distribution shape could be arbitrary. However, our wrapping method will accommodate any distribution shape that the practitioner thinks is appropriate, as long as it includes the base problem’s set of species that occur on exactly two PUs.

Once we have chosen the lognormal as our demonstration distribution, the question becomes which lognormal to use in each problem, i.e., what values are given to the lognormal's two parameters that control its shape? We address this choice by using an optimizer to search for the two parameters of a particular lognormal that i) doesn’t exceed a user-specified upper bound on total number of species, and ii) includes the base problem’s given number of species that occur on exactly two PUs (though it can include more species that occur on exactly two PUs).  

We guide the parameter search by providing an evaluation function that penalizes parameter choices that yield a lognormal exceeding a given maximum number of all species (so that problems don’t become unrealistically large) or that fall below the base problem’s number of species on two PUs. The choice of evaluation function to use is flexible in that it just needs to embody whatever constraints satisifies the user's beliefs about the appropriate function shape and somehow penalizes for violating them.  However, its form does have to differ from the squared error form used in regression because we only have data for the section of the curve containing species occurring on exactly two patches.  Everything else about the curve is underconstrained.  Though we use a lognormal as the target and our specific evaluation function to guide fitting, the same procedure works for choosing parameters of any wrapping distribution. Similarly, any desired evaluation function and optimization algorithm could be used. More details of the fitting procedure are given in CorSI.  
...
```{r predictRepShortfallUsingPUsAndSppOnly, fig.cap = "\\label{fig:predRepShortfallUsingPUsAndSppOnly}True representation shortfall vs. predicted representation shortfall on each problem in the test set using linear model trained for each reserve selector on input feature set containing only the number of planning units and the number of species.  Adjusted $R^2$ and rmse (on the test set) for each of the four learned prediction models are shown in the upper left corner of each plot.  Problems are colored by their dominant error type.  For reference, the line corresponding to a perfect fit of predicted to true values is shown diagonally upward from the origin."}
    ...
    #  Checkpoint 5 (planning/bdpg_fitting_refactor_plan.md): this is the one
    #  subsection wired to the new pipeline, behind params$use_new_fitting_pipeline
    #  (default FALSE -- old path below is byte-for-byte unchanged).  See
    #  DECISIONS.md.

if (params$use_new_fitting_pipeline)
    {
    new_pipeline_target_col_name = "rsr_COR_spp_rep_shortfall"

    new_pipeline_recipe_template_df = p3_train_x_df %>% select (-rs_method_name)
    new_pipeline_recipe_template_df [[new_pipeline_target_col_name]] =
        p3_working_train_df [[new_pipeline_target_col_name]]

    new_pipeline_result =
        run_output_error_fit (
            all_fitting_scores_df = all_fitting_scores_df,
            rs_method_names_list,
            train_x_df = p3_train_x_df, test_x_df = p3_test_x_df,
            working_train_df = p3_working_train_df, working_test_df = p3_working_test_df,
            train_aux_df = p3_train_aux_df, test_aux_df = p3_test_aux_df,
            target_col_name = new_pipeline_target_col_name,
            recipe       = make_bdpg_recipe (new_pipeline_recipe_template_df, new_pipeline_target_col_name),
            learner_spec = make_bdpg_learner (params$fitting_model_str),
            learner_id   = params$fitting_model_str,
            vars_used_str = vars_used_str, measure_name_str = "abs_rep_shortfall_resid",
            fitting_model_str = params$fitting_model_str,

            x_min_on_plot = NA, x_max_on_plot = NA,
            y_min_on_plot = -0.2, y_max_on_plot = 1.2,

            save_final_model = params$save_final_model)

    all_fitting_scores_df = new_pipeline_result$all_fitting_scores_df

        #  The current figure renders only because show() runs inside the old
        #  fit_and_predict_output_error_using_feature_set(); the new path must
        #  explicitly print the returned plot here so knitr captures it under
        #  this chunk's fig.cap.
    print (plot_output_error_fit (
        new_pipeline_result$bundle,
        ds_label = if (display_train_as_final_pred_using_plot) "TRAIN" else "TEST",
...
```{r predictSolCostErrorUsingPUsAndSppOnly, fig.cap = "\\label{fig:predSolCostErrorUsingPUsAndSppOnly}True solution cost error vs. predicted solution cost error on each problem in the test set using linear model trained for each reserve selector on input feature set containing only the number of planning units and the number of species.  Adjusted $R^2$ and rmse (on the test set) for each of the four learned prediction models are shown in the upper left corner of each plot.  Problems are colored by their dominant error type.  For reference, the line corresponding to a perfect fit of predicted to true values is shown diagonally upward from the origin"}
    ...
    #  Checkpoint 6 (planning/bdpg_fitting_refactor_plan.md): wired to the
    #  new pipeline behind params$use_new_fitting_pipeline, same pattern as
    #  Checkpoint 5.  See DECISIONS.md.

if (params$use_new_fitting_pipeline)
    {
    new_pipeline_target_col_name = "rs_solution_cost_err_frac"
    ...
    new_pipeline_result =
        run_output_error_fit (
            ...
            learner_spec = make_bdpg_learner (params$fitting_model_str),
            learner_id   = params$fitting_model_str,
            vars_used_str = vars_used_str, measure_name_str = "abs_sol_cost_err_resid",
            fitting_model_str = params$fitting_model_str,
...
```{r captureGoldenMasterInputsAndScores, include=TRUE}

#  Golden-master capture for the fitting/eval refactor bootstrap (see
#  planning/bdpg_fitting_refactor_plan.md, "Golden-master specifics").  ADDITIVE and
#  INERT when params$capture_golden_master is FALSE (the default) -- no behavior
#  change to normal knits.  The overwrite guard earlier in this document has already
#  aborted the run if a finalized golden record exists and
#  params$force_golden_master is not set.
#
#  Writes two distinct things, both as RDS via the same dataprov idiom already used
#  for the prep-run outputs (write_a_tib_with_provenance() / resolve_prov_file()):
#    - the frozen INPUT snapshot: the working + aux frames exactly as they exist at
#      this point in the document, i.e. AFTER the Gurobi->ILP renaming above, not the
#      on-disk pre-munge CSVs -- into tests/fixtures/inputs/
#    - the frozen OUTPUT golden: all_fitting_scores_df, accumulated across every fit
#      call in this document (not hardcoded here) -- into tests/fixtures/golden/
#
#  This is a one-shot bootstrap (see plan): once the new tidymodels pipeline is
#  validated against this golden, the old pipeline and this capture instrumentation
#  are retired.
...
if (isTRUE (params$capture_golden_master))
    ...
    golden_registry = open_or_init_registry (golden_registry_dir)

    golden_sess = prov_session_start (
        golden_registry,
        description = paste0 ("bdpg fitting-refactor golden-master capture; ",
                              "gurobi_problem_filter=", params$gurobi_problem_filter,
                              "; exclude_imperfect_wraps=", params$exclude_imperfect_wraps),
        parameters  = params
        )

    write_a_tib_with_provenance (golden_sess, p3_working_train_df,
                                 "p3_working_train_df", inputs_dir,
                                 params, "rds")
    write_a_tib_with_provenance (golden_sess, p3_working_test_df,
                                 "p3_working_test_df", inputs_dir,
                                 params, "rds")
    write_a_tib_with_provenance (golden_sess, p3_train_aux_df,
                                 "p3_train_aux_df", inputs_dir,
                                 params, "rds")
    write_a_tib_with_provenance (golden_sess, p3_test_aux_df,
                                 "p3_test_aux_df", inputs_dir,
                                 params, "rds")

    write_a_tib_with_provenance (golden_sess, all_fitting_scores_df,
                                 "all_fitting_scores_df", golden_dir,
                                 params, "rds")

    prov_session_close (golden_sess)

    cat ("\n\n=========================================================\n")
    message ("GOLDEN MASTER CAPTURE SESSION UUID: ", golden_sess$uuid)
    cat ("GOLDEN MASTER CAPTURE SESSION UUID: ", golden_sess$uuid, "\n", sep = "")
    cat ("nrow (all_fitting_scores_df) = ", nrow (all_fitting_scores_df), "\n", sep = "")
    cat ("dim (p3_working_train_df)   = ", paste (dim (p3_working_train_df), collapse = " x "), "\n", sep = "")
    cat ("dim (p3_working_test_df)    = ", paste (dim (p3_working_test_df), collapse = " x "), "\n", sep = "")
    cat ("dim (p3_train_aux_df)       = ", paste (dim (p3_train_aux_df), collapse = " x "), "\n", sep = "")
    cat ("dim (p3_test_aux_df)        = ", paste (dim (p3_test_aux_df), collapse = " x "), "\n", sep = "")
    cat ("=========================================================\n\n")

    DBI::dbDisconnect (golden_registry$conn)
    }
```
