...
**Audience:** a Claude Code session working in the `bdpgtext2` repo. **Status:** design-complete handoff. Implement in the staged, checkpointed order below. Stop at the end of each checkpoint and wait for the human (referred to here as "the author") to review before proceeding. **Hard rule:** this is a *parallel* build. Do not modify working code or the analysis Rmd's existing behavior until the explicit wiring checkpoints (5–6), and only then behind a switch with the old path still callable.
...
- **Frozen *inputs*** (Checkpoint 0) — a stable copy of the working frames the pipeline consumes: `p3_working_train_df` / `p3_working_test_df` plus the aux frames `p3_train_aux_df` / `p3_test_aux_df`. These already exist (the prep Rmd produces them and the analysis Rmd reads them). The snapshot is taken **in memory at the fit call site as RDS** (details in Checkpoint 0 and "Golden-master specifics" below), so the new pipeline's equivalence test has a fixed input to run against.\
- **Frozen *output* = the metric golden** (Checkpoint 1) — the `adj_R2` / `rmse` / `R2` in `all_fitting_scores_df`, produced by running the **old** pipeline on those inputs. This is the reference the new code must reproduce. **Only metrics are captured from the old side** (see "Golden-master specifics" for why predictions are not).

Division of labor: the old-side capture is an **additive, flag-gated instrumentation of the existing analysis Rmd**, not an external script. Claude Code may draft the flag-gated chunk; the **author reviews it and runs the Rmd once with the capture flag on** (trusted environment) to produce and commit the canonical metric golden. This is a deliberate, scoped exception to the no-modify rule — additive and inert when the flag is off — after which the refactor resumes no-modify (see "Golden-master specifics").

- **Captured from the OLD code, one-shot**, in the author's trusted environment. The old pipeline stays live only long enough to validate the new one; this is a sanity gate ("the new pipeline isn't doing something obviously wrong"), not a durable facility.\
- **The old side has a single tier — the full-batch metric golden.** The fast inner loop Claude Code needs for TDD is served instead by a **new-side self-regression fixture** on a deterministic COR-subsample (the new fitting function returns its own values, so no old code is involved). The subsample-selection rule still applies to that fixture (see "Golden-master specifics").\
- **LM equivalence test:** the new core reproduces the golden `adj_R2` / `rmse` / `R2` within tight tolerance (LM via parsnip wraps `stats::lm`, so expect near-exact).\
...
These fix the mechanics the bullets above leave open. **Superseding note:** an earlier draft of this section had the old pipeline capture *predictions* as ground truth via an external script, with a two-tier old-side golden. That is replaced by what follows — metrics-only, captured in-Rmd, one-shot.

- **Metrics only, from the old side.** Capture just `all_fitting_scores_df` (rmse / R² / adj-R² per reserve selector × feature set × error type, TRAIN and TEST). Old-side *predictions* are deliberately not captured: at Rmd scope only the scores survive — the per-cell `true_values` / `pred_values` are discarded inside `fit_and_predict_output_error_using_feature_set()` — so reaching them would require editing the old fitting function's logic (a real modification, not inert instrumentation) or a redundant re-fit. For a "not obviously wrong" sanity gate, a per-cell metric match to `~1.5e-8` is sufficient. Any predictions golden is a **new-side self-regression fixture** (free, since the new fitting function returns the bundle) and is not part of the old-vs-new gate.

- **Equivalence tolerance.** Assert the metrics at testthat's default tolerance (`~1.5e-8`). With an identically built design matrix, LM-via-parsnip agreement should be near machine precision. **If the tolerance has to be loosened to pass, do not loosen it — investigate.** The likely culprit is design-matrix construction at the recipe seam (intercept handling, factor contrasts, or column ordering differing between the pass-through recipe/hardhat path and the old code's manual `as.matrix` / `model.matrix`); the tight tolerance doubles as a correctness probe on that seam.

- **Capture mechanism: in-Rmd, flag-gated, provenance-tracked.** Add a params flag (e.g. `capture_golden_master`, default `FALSE`) to the analysis Rmd plus one additive capture chunk after the last fit chunk. Off → it does nothing (no behavior change to normal builds). On → it writes `all_fitting_scores_df` via `write_a_tib_with_provenance(sess, …)` **as RDS** into the golden fixture area, reusing the existing dataprov session/registry (`Data/dataprov_registry`) already used for the prep outputs. RDS preserves types; provenance lives in the registry, so the payload stays clean and diff-deterministic and reads are hash-verified via `resolve_prov_file()`.

- **Overwrite guard: existence check + `force`, not a prompt.** Evaluate at the *start* of the run, before any fitting or writes: if the capture flag is on and the golden already exists and `force` is not set, `stop()` immediately (fail-fast; no partial or misleading trail). An interactive prompt is avoided because `interactive()` is `FALSE` under `rmarkdown::render()` and `readline()` would not block. This turns the accidental-overwrite case (flag left on) into a clean abort and makes a "generate = yes but nothing generated" contradictory record impossible. (An `if (interactive())` confirmation may be layered on as a second guard, never as the sole one.)

- **Fast inner-loop fixture (new-side).** For Claude Code's TDD loop, subsample by COR group: fixed seed, then the *smallest* set of distinct `rsp_UUID_of_COR_Base_problem_that_is_wrapped` values that still (a) populates all 4 reserve selectors × 2 error types, (b) retains both `dom_err_type` levels, and (c) leaves every cell non-degenerate for an LM fit (`n > p + 1`, not rank-deficient). The golden for this fixture is the **new pipeline's own output** (self-regression), not the old pipeline's; statistical representativeness of the science is explicitly not a goal. Selecting on the COR group (not random rows) keeps the fixture usable for FC-1's grouped CV.

- **Scope: one-shot bootstrap.** This whole facility is a transient bootstrap to confirm the new pipeline against the old, not durable infrastructure — once the new pipeline is confirmed, the old pipeline and this capture instrumentation are retired. If the golden needs a legitimate refresh during the transition, re-run the Rmd with the flag on and `force`, review the committed golden's git diff and its provenance record, and note it in `DECISIONS.md`.
...
- **Freeze the input snapshot (Checkpoint 0 = inputs only, not the golden).** Snapshot the **working frames plus aux** — `p3_working_train_df` / `p3_working_test_df` and `p3_train_aux_df` / `p3_test_aux_df` — as **RDS** into `FIXTURE_ROOT/inputs/`, and commit them, so the tests have a fixed input that does not move when data is regenerated. Snapshotting the *working* frames (rather than the already-derived `p3_train_x_df` / `p3_test_x_df`) is deliberate: one snapshot serves every feature set, and the fixture's own `build_feature_set_specific_test_and_train()` derives the per-feature-set x-frames, keeping that derivation inside the tested path.
  ...
  **Commit the full batch.** Measured sizes: each working frame is \~13 MB as CSV (smaller as compressed RDS) and each aux frame \~100 KB — small enough to commit directly (no Git LFS). The full-batch input snapshot is committed for the new pipeline's full-batch equivalence run; the COR-subsample used for the new-side inner-loop fixture is derived deterministically from those committed inputs by the rule in "Golden-master specifics". The input snapshot is written by the same flag-gated capture instrumentation described there (inputs + scores under one flag). This step freezes inputs; the metric golden comes in Checkpoint 1.\
...
This checkpoint freezes the old pipeline's **metric output** (the golden master) — distinct from Checkpoint 0, which froze the **inputs**. Same frozen inputs go in; the old code's `all_fitting_scores_df` comes out and becomes the reference.

- **Instrument the analysis Rmd additively (flag-gated).** Claude Code may draft a `capture_golden_master` params flag (default `FALSE`) plus one capture chunk after the last fit chunk; the **author reviews and runs it**. When on, the chunk writes `all_fitting_scores_df` via `write_a_tib_with_provenance(sess, …)` as RDS into `FIXTURE_ROOT/golden/`, reusing the prep run's dataprov session/registry idiom. When off, it is inert. This is the scoped, additive exception to the no-modify rule; log it in `DECISIONS.md`.\
- **Overwrite guard runs first.** At the top of the run, if the flag is on and the golden exists and `force` is not set, `stop()` before any fitting or writes (see "Golden-master specifics").\
- **The author runs the capture once, on the full batch** (trusted environment, old code otherwise untouched), and commits the result — the paper's authoritative numbers trace to the author's environment, captured before any new code exists. This is a one-shot; there is **no old-side subset golden** (the fast inner loop uses a new-side self-regression fixture, created once the new pipeline exists).\
- Note: the capture chunk needs the old `show()` side effect suppressed or redirected; capture the scores tibble, not figures.\
- **DoD:** the full-batch metric golden RDS exists under `FIXTURE_ROOT/golden/` with a provenance record; a test loads it (hash-verified); the overwrite guard aborts a re-run when `force` is unset.
...
- **LM equivalence test:** the new pipeline's metrics reproduce the old-side full-batch metric golden within `~1.5e-8`. The fast inner loop uses a **new-side self-regression** fixture on the COR-subsample (new pipeline vs its own frozen values).\
- **RF smoke test** through the same path (structure only).\
- **DoD:** LM metric equivalence green against the full-batch golden; new-side self-regression green on the subset; RF smoke green; no plotting code present in this function.
