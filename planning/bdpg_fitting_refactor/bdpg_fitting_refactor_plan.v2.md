...
- **Frozen *inputs*** (Checkpoint 0) — a stable copy of the `(train_df, test_df)` that the pipeline consumes. These already exist (the prep Rmd produces them and the analysis Rmd reads them); the only new work is placing a snapshot copy under version control in the test tree, because the project's data is otherwise not committed.  
...
- **Freeze the input snapshot (Checkpoint 0 = inputs only, not the golden).** The prepared `p3_working_train_df` and `p3_working_test_df` already exist on disk (the prep Rmd writes them; the analysis Rmd reads them). Copy those exact frames into `FIXTURE_ROOT/inputs/` as **RDS** (preserves types, factor levels, exact values) and commit them, so the tests have a fixed input that does not move when data is regenerated. This is a *snapshot copy, not a regeneration*: Claude Code can perform the copy itself once the author points it at the source path of the saved frames (or the author can drop the two files in). This step freezes inputs only — the golden outputs come in Checkpoint 1.  
